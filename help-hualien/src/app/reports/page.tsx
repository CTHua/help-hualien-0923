...
export default function Reports() {
      ...
      (error) => {
        ...
        switch (error.code) {
          case error.PERMISSION_DENIED:
            errorMessage =
              "使用者拒絕了地理定位請求。請在瀏覽器設定中允許位置存取權限。";
            break;
          case error.POSITION_UNAVAILABLE:
            errorMessage =
              "位置訊息不可用。請確保您的裝置支援GPS定位，或嘗試移動到空曠的地方。";
            break;
          case error.TIMEOUT:
          ...
          default:
            errorMessage =
              "獲取位置時發生未知錯誤。請確保您的裝置支援定位功能並已開啟GPS。";
            break;
        }
  ...
  const reportOngoing = async () => {
    if (
      ...
      !selectedReportId ||
      !estimatedMinutes ||
      ongoingReports.has(selectedReportId)
    )
      return;
    ...
    try {
      ...
      const response = await fetch(
        "https://help-hualien-api.cthua.io/ongoing",
        {
          method: "POST",
          headers: {
                  ...
                  <div>
                    ...
                    <div className="flex space-x-4 text-sm">
                      <span className="text-blue-600">
                        前往中: {report.onGoingCount}
                      </span>
                      <span className="text-green-600">
                        已到達: {report.arrivedCount}
                      </span>
                      <span className="text-gray-600">
                        已離開: {report.leftCount}
                      </span>
                    </div>
                  </div>
                </div>
